...
**v1.27 corrige `/ably-token`: daba 502 siempre, en cualquier hosting con conexiones salientes bloqueadas desde PHP.**

- La v1.25 original le pedía el token a Ably llamando a su API por `wp_remote_post()` — una conexión de PHP HACIA AFUERA. En Hostinger (y hosting compartido en general) eso suele estar bloqueado o no responder, y el síntoma no tiene nada que ver con el negocio ni la API Key: Cloudflare devuelve 502 "the origin web server returned an invalid or incomplete response" en TODAS las llamadas, sin excepción.
...
- **`POST /wp-json/punto/v1/ably-token`** — con `Authorization: Bearer <token de cuenta>`. Exige negocio activo (mismo candado que `/stock`); sin eso, 403 con `requiere_activacion:true`. Devuelve `{token}` — un JWT válido 1 hora, con permiso **solo** sobre el canal `negocio-{id}` de esa cuenta — ni ve ni puede tocar el canal de otro negocio. (Desde v1.27 el JWT se firma local, sin llamar a Ably — ver esa entrada, más abajo quedó como estaba escrita en su momento la versión que SÍ llamaba a Ably por HTTP.)
- **La API Key completa de Ably no sale nunca del servidor.** Se guarda en `wp_options` (`punto_ably_api_key`) desde el panel nuevo **Punto → Tiempo real**, y de ahí este endpoint la usa para firmar el JWT.
...
- **El cliente (POS) ya lo consume**, del lado del tema hijo (`astra-child/page-pos.php` + el SDK vendorizado en `astra-child/pos/ably.min.js`, para que la PWA siga siendo instalable/offline sin depender de un CDN de terceros). Se conecta con `authUrl: /wp-json/punto/v1/ably-token` (mandando el `Authorization` de siempre) en vez de una API Key, se suscribe al canal `negocio-{id}` una vez logueado con negocio activo, y publica un cambio (`emitCambio()`) cuando se guarda/borra un producto, cliente, categoría, egreso o un grupo de Configuración — **NO** en Perfil, que es dato de cuenta, no de negocio. Al recibir uno de otro dispositivo (en vivo por Ably, o poniéndose al día con `/cambios`), lo aplica con `aplicarCambioRemoto()`: upsert por id para catálogo/clientes/categorías/egresos, y `applyGroup()` (la misma función del botón "Descartar cambios") para Configuración, para no reinventar mal el mapeo de cada grupo. Ver v1.26 arriba para la garantía de entrega cuando el otro dispositivo estaba desconectado.
- **`clients` y `egresos` pasaron a usar id único global** (`'c_'+uuidLite()` / `'e_'+uuidLite()`), igual que los productos desde v1.8 — necesario para que el upsert por id entre dispositivos no confunda dos registros distintos creados offline con el mismo id secuencial. Los ya existentes (con id viejo tipo `'c101'`) seguían funcionando igual, solo los nuevos usan el id global.
